...
let pitch = 0, isGameStarted = false, isDead = false;
...
function init() {
    ...
    window.addEventListener('mousedown', () => { if (isGameStarted && !isDead) shoot(); });
...
function startGame() {
    ...
    // ПРАВКА: Интервальный спавн
    setInterval(() => { if (monsters.length < 15) spawnRandomMonster(); }, 5000);
}
...
function spawnRandomMonster() {
    ...
    gltfLoader.load(path + 'assets/sprites/models/monster.glb', (gltf) => {
        ...
        model.userData = { health: 100, speed: 0.045, mixer: gltf.animations.length > 0 ? mixers[mixers.length-1] : null };
        monsters.push(model);
    });
}
...
function animate() {
    ...
    // ПРАВКА: Коллизии со стенами и монстрами (камера не проваливается)
    let collision = false;
...
function shoot() {
    ...
    if (shotSound && shotSound.buffer) shotSound.play();
    ...
    if (intersects.length > 0) {
        ...
        if (obj && obj.userData.health) {
            ...
            if (obj.userData.health <= 0) {
                scene.remove(obj);
                if (obj.userData.mixer) mixers = mixers.filter(mix => mix !== obj.userData.mixer);
                monsters = monsters.filter(m => m !== obj);
                kills++; document.getElementById('kill-counter').innerText = "УБИТО: " + kills;
            }
        }
    }
    setTimeout(() => { isShooting = false; if (pistolMag === 0) reloadPistol(); }, 100);
}
